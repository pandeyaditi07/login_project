...
$selectquery = " select * from users2 where id='$id'";

$query = mysqli_query($con , $selectquery ); 

$result = mysqli_fetch_assoc($query);
// $resu = mysqli_num_rows($query); 
...
if(isset($_POST['submit'])){
	...
	$updatequery = "update users2 set  name='$name', username = '$username' , password = '$password'  where id='$id' ";
...
<div class="input-card text-center">
    ...
    <input type="text" placeholder="Enter your name" class="input" name="name" value=" <?php  echo $result['name'];  ?>"  required> 
    <input type="text" placeholder="Enter username" class="input" name="username" value=" <?php  echo $result['username'];  ?>" required>
    <input type="password" placeholder="********" class="input"  name="password"  value="<?php  echo $result['password'];  ?>" required> 
	<!-- <input type="update" class="btnRegister"  name="submit" value="update"/>  -->
    <!-- <button class="btn" type="" value="update" name="submit1">update</button>   -->
	<button class="btn-update" style="background-color: pink;" id="btn-update" type="update" name="submit">update</button>  
...
<!-- </div>  --> 
<!-- <button class="btn-update1" style="background-color: pink;" id="btn-update" type="update" name="submit">update</button>   -->
